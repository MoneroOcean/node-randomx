...
module.exports.cluster_process = function() {
  ...
  function send_msg(type, value) {
    return process.send({type, value, thread_id});
  }
...
module.exports.record_thread = function(worker_id) {
  if (worker_ids.includes(worker_id)) return;
  worker_ids.push(worker_id);

  const worker = cluster.workers && cluster.workers[worker_id];
  if (worker) {
    worker.once("exit", function() {
      setImmediate(function() { forget_thread(worker_id); });
    });
  }
};

module.exports.is_compute_thread = function(worker_id) {
  return worker_ids.includes(worker_id);
};

module.exports.create_thread = function(messageHandler) {
  const compute_thread_id = next_thread_id();
  const thread = cluster.fork({thread_id: String(compute_thread_id)});
...
module.exports.get_thread_dev = function(thread_index, devs) {
